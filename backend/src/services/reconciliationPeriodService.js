...
const calculatePeriodDates = (transactionDate) => {
    ...
    if (date < FIRST_PERIOD_START) {
        throw new Error("Transaction date is before the first reconciliation period");
    }
    const daysSinceStart = Math.floor(
        (date - FIRST_PERIOD_START) / MS_PER_DAY
    );
    const periodIndex = Math.floor(daysSinceStart / PERIOD_LENGTH_DAYS);
    const startDate = addDays(FIRST_PERIOD_START, periodIndex * PERIOD_LENGTH_DAYS);
